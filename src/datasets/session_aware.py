...
class SessionAwareDataset(Dataset):
    """
    Abstract class for session-aware next-item recommendations
    """
    ...
    def __init__(
        ...
        if name == "train":
            ...
            # we perform leave-one-out split on the val sessions
            assert val_q is not None, "Validation set is empty"
            assert val is not None, "Validation set is empty"
            val_items = val[~val.duplicated(subset="session_id", keep="last")]
            val_items = dict(zip(val_items["session_id"], val_items["item_id"]))
            val = val[val.duplicated(subset="session_id", keep="last")]
            self._df = val.copy()
            users = dict(zip(self._df["session_id"], self._df["uid"]))
            index = [
                {
    ...
    def _load_data(self):
        """
        Download/load and return a dataframe for the dataset.
        It should contain "uid", "item_id" and "timestamp" (in seconds) columns
        """
